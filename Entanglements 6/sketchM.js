  ...
  p.drawLandmarks = function (indexArray, hue) {
    ...
    for (let i = 0; i < detections.multiHandLandmarks.length; i++) {
      for (let j = indexArray[0]; j < indexArray[1]; j++) {
        ...
        // let z = detections.multiHandLandmarks[i][j].z;
        p.stroke(255); //hue, 40, 255
        p.point(p.windowWidth - x, y);
        if (j == 9) {
          xLocator = detections.multiHandLandmarks[i][j].x * p.windowWidth;
          yLocator = detections.multiHandLandmarks[i][j].y * p.windowHeight;
        }
      }
    }
  };
  ...
  p.drawLines = function (index) {
    ...
    for (let i = 0; i < detections.multiHandLandmarks.length; i++) {
      for (let j = 0; j < index.length - 1; j++) {
        ...
        p.line(p.windowWidth - x, y, windowWidth - _x, _y);
      }
    }
  };
};
